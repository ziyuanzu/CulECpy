...
def ECM_FBA(Concretemodel_Need_Data,a,x,y,mid_a,mid_b,bio_a,bio_b,obj_name,E_total=0.227,biomass_id='BIOMASS_Ec_iML1515_core_75p37M'):#基于pyomo的FBA分析方法（glc）
...
def ECM_pFBA(Concretemodel_Need_Data,a,x,y,obj,mid_a,mid_b,bio_a,bio_b,obj_name,E_total=0.227,biomass_id='BIOMASS_Ec_iML1515_core_75p37M'):#基于pyomo的最小通量和pFBA分析方法（glc）
    substrates={'EX_glc__D_e_reverse':a}
    substrate_value=10
    substrate_name=''
    E_total=E_total
    ...
    #biomass_id='BIOMASS_Ec_iML1515_core_75p37M'
    #pass_value={'A_tyr__L_e_con_1':tyr,'B_phpyr_e_con_2_reverse':php}
    EcoECM_pFBA=Template_Concretemodel_double(reaction_list=reaction_list,metabolite_list=metabolite_list,coef_matrix=coef_matrix,\
        reaction_list_A=reaction_list_A,reaction_list_B=reaction_list_B,reaction_kcat_MW=reaction_kcat_MW,lb_list=lb_list,ub_list=ub_list,\
...
def ECM_FBA_subs(Concretemodel_Need_Data,subs,x,y,mid_a,mid_b,bio_a,bio_b,obj_name,E_total=0.227,biomass_id='BIOMASS_Ec_iML1515_core_75p37M'):#基于pyomo多底物的FBA分析方法
    a,b=subs
...
def ECM_pFBA_subs(Concretemodel_Need_Data,subs,x,y,obj,mid_a,mid_b,bio_a,bio_b,obj_name,E_total=0.227,biomass_id='BIOMASS_Ec_iML1515_core_75p37M'):#基于pyomo的最小通量和多底物pFBA分析方法
    a,b=subs
    substrates={'EX_glc__D_e_reverse':a,'EX_xyl__D_e_reverse':b}
    substrate_value=10
    substrate_name=''
...
def fba_max_product(Concretemodel_Need_Data,double,sub,mid_a,mid_b,bio_a,bio_b,obj_name):#不同菌株比例、底物比例设定下目标产品最大合成通量梯度模拟
    pass_v=0
    (x,y)=double
    a=ECM_FBA(Concretemodel_Need_Data,sub,x,y,mid_a,mid_b,bio_a,bio_b,obj_name)
...
def pfba_max_product(Concretemodel_Need_Data,model_file,double,sub,mid_a,mid_b,bio_a,bio_b,obj_name):#不同菌株比例、底物比例设定下目标产品最大合成通量梯度模拟及最小通量和全局通量模拟
    (x,y,obj,filename)=double
    model_out=load_json_model(model_file)
    cobra.manipulation.modify.convert_to_irreversible(model_out)
    isoenzyme_split(model_out)
    b=ECM_pFBA(Concretemodel_Need_Data,sub,x,y,obj,mid_a,mid_b,bio_a,bio_b,obj_name)
    ...
    model_pfba_solution_detail.to_csv(filename)
    return model_pfba_solution_detail

def fba_max_subs_product(Concretemodel_Need_Data,double,subs,mid_a,mid_b,bio_a,bio_b,obj_name):#不同菌株比例、底物比例设定下目标产品最大合成通量梯度模拟（多底物）
    pass_v=0
    (x,y)=double
    a=ECM_FBA_subs(Concretemodel_Need_Data,subs,x,y,mid_a,mid_b,bio_a,bio_b,obj_name)
    print('A:B='+str(x)+':'+str(y)+'→obj:')
    print(a.obj())
    # a_obj=get_two_float(a.obj(),2)
    # b=ECM_pFBA(100,list_x[i],list_y[i],a_obj,0,0,0.1,0.1)
...
def pfba_max_subs_product(Concretemodel_Need_Data,model_out,double,obj,subs,mid_a,mid_b,bio_a,bio_b,obj_name):#不同菌株比例、底物比例设定下目标产品最大合成通量梯度模拟及最小通量和全局通量模拟（多底物）
    (x,y)=double
    b=ECM_pFBA_subs(Concretemodel_Need_Data,subs,x,y,obj,mid_a,mid_b,bio_a,bio_b,obj_name)
    reaction_kcat_mw=Concretemodel_Need_Data['reaction_kcat_MW']
    model_pfba_solution_detail=pd.DataFrame()
    for i in b.reaction:
        model_pfba_solution_detail.loc[i,'fluxes'] = b.reaction[i].value
